---
title: "Datenschutzerklärung"
description: "Informationen zum Datenschutz bei Oberbeck Marketing"
pubDate: 2026-04-04
---

# Datenschutzerklärung

## 1. Verantwortlicher

**Oberbeck Marketing**  
Philipp Oberbeck  
Hauptstraße 123  
67149 Laumersheim  
Deutschland

**Kontakt:**  
E-Mail: kontakt@oberbeck-marketing.de  
Telefon: 06238 1234567

## 2. Allgemeine Hinweise

Wir nehmen den Schutz Ihrer persönlichen Daten sehr ernst. Diese Datenschutzerklärung informiert Sie darüber, welche Daten wir erheben, wie wir sie verwenden und welche Rechte Sie haben.

## 3. Erhebung und Verarbeitung personenbezogener Daten

### 3.1 Bei Besuch unserer Website

Beim Besuch unserer Website werden folgende Daten automatisch erfasst:

- IP-Adresse (anonymisiert)
- Datum und Uhrzeit des Zugriffs
- Browsertyp und -version
- Verwendetes Betriebssystem
- Referrer-URL
- Aufgerufene Seiten

**Rechtsgrundlage:** Berechtigtes Interesse (Art. 6 Abs. 1 lit. f DSGVO)

### 3.2 Kontaktformular

Wenn Sie uns über das Kontaktformular schreiben, erheben wir:

- Name
- E-Mail-Adresse
- Telefonnummer (optional)
- Ihre Nachricht

**Zweck:** Bearbeitung Ihrer Anfrage  
**Rechtsgrundlage:** Vertragserfüllung/Vertragsanbahnung (Art. 6 Abs. 1 lit. b DSGVO)

### 3.3 Newsletter

Für den Newsletterversand erheben wir:

- E-Mail-Adresse
- Einwilligungsnachweis

**Rechtsgrundlage:** Einwilligung (Art. 6 Abs. 1 lit. a DSGVO)  
**Speicherdauer:** Bis zur Abmeldung

## 4. Cookies und Tracking

### 4.1 Notwendige Cookies

Diese Cookies sind technisch erforderlich:

- Session-Cookies
- Cookie-Einwilligungs-Speicherung

### 4.2 Analyse- und Marketing-Cookies

Mit Ihrer Einwilligung verwenden wir:

**Google Analytics 4**
- Zweck: Website-Analyse
- Anbieter: Google Ireland Limited
- Datenschutz: [Google Privacy Policy](https://policies.google.com/privacy)
- IP-Anonymisierung: Aktiviert

**Meta Pixel (Facebook)**
- Zweck: Werbeanalyse
- Anbieter: Meta Platforms Ireland Limited
- Datenschutz: [Meta Privacy Policy](https://www.facebook.com/privacy/policy)

## 5. Datenweitergabe

Wir geben Ihre Daten nur weiter an:

- **Hosting-Provider** (für Website-Betrieb)
- **E-Mail-Provider** (für Kommunikation)
- **Analyse-Tools** (Google, Meta - nur mit Einwilligung)

Eine Übermittlung in Drittländer (außerhalb der EU) erfolgt nur mit angemessenen Garantien (Standardvertragsklauseln).

## 6. Speicherdauer

Ihre Daten werden gespeichert:

- **Kontaktdaten:** Bis zur vollständigen Bearbeitung Ihrer Anfrage
- **Vertragsdaten:** 10 Jahre (gesetzliche Aufbewahrung)
- **Analyse-Daten:** 26 Monate (Google Analytics)
- **Newsletter-Daten:** Bis zur Abmeldung

## 7. Ihre Rechte

Sie haben folgende Rechte:

### 7.1 Auskunft (Art. 15 DSGVO)
Sie können jederzeit Auskunft über Ihre bei uns gespeicherten Daten verlangen.

### 7.2 Berichtigung (Art. 16 DSGVO)
Sie haben das Recht, unrichtige Daten korrigieren zu lassen.

### 7.3 Löschung (Art. 17 DSGVO)
Sie können die Löschung Ihrer Daten verlangen ("Recht auf Vergessenwerden").

### 7.4 Einschränkung (Art. 18 DSGVO)
Sie können die Verarbeitung einschränken lassen.

### 7.5 Datenübertragbarkeit (Art. 20 DSGVO)
Sie können Ihre Daten in einem maschinenlesbaren Format erhalten.

### 7.6 Widerspruch (Art. 21 DSGVO)
Sie können jederzeit gegen die Verarbeitung widersprechen.

### 7.7 Beschwerderecht (Art. 77 DSGVO)
Sie haben das Recht, sich bei einer Aufsichtsbehörde zu beschweren.

**Zuständige Aufsichtsbehörde:**  
Der Landesbeauftragte für den Datenschutz und die Informationsfreiheit Rheinland-Pfalz  
Postfach 30 40, 55020 Mainz  
E-Mail: poststelle@datenschutz.rlp.de

## 8. Datensicherheit

Wir verwenden folgende Sicherheitsmaßnahmen:

- SSL-Verschlüsselung (HTTPS)
- Regelmäßige Sicherheitsupdates
- Zugriffsbeschränkungen
- Datensparsamkeit

## 9. Änderungen dieser Datenschutzerklärung

Wir behalten uns vor, diese Datenschutzerklärung anzupassen. Bitte informieren Sie sich regelmäßig über Änderungen.

**Stand:** April 2026